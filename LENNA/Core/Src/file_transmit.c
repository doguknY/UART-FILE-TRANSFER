...
uint8_t init_file_transmit(uint8_t *transmission_info_message) {
    ...
    file_size = get_file_size(NAME_OF_TRANSMITTED_FILE_TO_GROUND);
    if (file_size == 0 || fr != FR_OK) {
        return fr;  // Dosya okunurken hata
    }
    total_chunk_number = get_total_chunk_number(NAME_OF_TRANSMITTED_FILE_TO_GROUND);
...
void pack_chunk(uint8_t *chunk_to_send) {
    ...
    fr = read_from_anywhere(chunk_to_send + FIRST_FILE_DATA_INDEX, data_size_to_read, which_chunk_to_send.u16 * FILE_SIZE_ON_CHUNK, NAME_OF_TRANSMITTED_FILE_TO_GROUND);
